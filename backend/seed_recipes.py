    ...
    with app.app_context():
        ...
        for r in RECIPES:
            exists = db.session.execute(
                db.select(Recipe).where(Recipe.name == r['name'], Recipe.user_id == user.id)
            ).scalar_one_or_none()
            if exists:
                print(f'Skipping: {r["name"]}')
                continue

            recipe = Recipe(name=r['name'], description=r['description'], user_id=user.id)
            ...
            for food_name, quantity in r['ingredients']:
                ...
                if not food:
                    ...
                    continue
                ingredient = Ingredient(food_id=food.id, quantity=quantity, recipe_id=recipe.id)
